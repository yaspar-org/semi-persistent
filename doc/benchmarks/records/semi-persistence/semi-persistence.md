...
This record quantifies the value of semi-persistence. It contains the mechanism
survey, four
...
Read `doc/benchmarks/methodology.md` first. Every divergence this pass introduces is
registered there.
...
full index was. Section 12 carries the superseding per-phase and wall-time
numbers. The "ours, naive" and "egglog"
...
`calc.egg` is used instead of herbie.egg. The latter is
...
The tables use fixed binary copies: `semi-persistent` md5
038284b34342ebc52148956704677c83 at source revision 413f590 plus the source
patch identified by that binary, and `egglog` md5
9213d10fe3777cce331a6eba317e3945 at revision 7b1adf2. A separate pass reproduced
sections 3, 4 and 6 within 8%; final comparison tables follow the source and
binary pinning rules in methodology.md section 6.
...
and `egraph/src/literal.rs` and were built three minutes apart from the same
source snapshot: `semi-persistent` md5
...
cd comparison
python3 scripts/span-table.py --phases \
    --prog semi-persistence/sp-t100000.cycles.native.egg --bin now=<binary>
python3 scripts/span-table.py --wall --runs 5 --warmups 2 \
    --prog semi-persistence/sp-t100000.cycles.native.egg --bin now=<binary>
```
