...
> Archived campaign. `final-r6-tables.md` is the newest retained historical
> campaign; none of these files measures the current implementation.
...
- **Semi-naive derives completely** (the class-growth fix): its node counts on
  ...
  strategy silently skipped derivations. A per-round merged-class set consulted
  by the delta variants, instead of member nodes pushed into the touched log per
  merge, is the documented alternative.
