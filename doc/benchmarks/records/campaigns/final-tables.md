...
> Archived campaign (r1). `final-r6-tables.md` is the newest retained
> historical campaign; the numbers here predate the engine changes recorded in
> `../methodology.md` section 6, and neither campaign measures the current
> implementation.
...
This campaign was produced by the runner in the `comparison/` layout, which
`scripts/egglog-compare/compare.py` replaced. The command below is what ran at the
time and no longer resolves; the current equivalent is
`python3 scripts/egglog-compare/compare.py --label final --runs 10 --warmups 2`,
which reproduces the protocol but not necessarily these numbers, since the engine
has moved since.
