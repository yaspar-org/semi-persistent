...
The numbers are from one machine and the binaries named under "Provenance".
`scripts/egglog-compare/gen-addac-sweep.py` regenerates the
programs. The sweep's own timing runner was retired with the `comparison/` layout and
has no replacement: reproducing the table means timing the generated programs directly,
keeping the blocking-wait split described under "Timing".
...
shipped default. Semi-naive is not reported because its access-path behavior on
this workload has not been established.
...
what puts `f64` in scope. This matches the campaign harness
...
flat, and the growth disappears into the quantization. The runner therefore
passed `timeout=` only on the one bounded run that decides completion or DNF, and timed
...
taken under sustained background CPU load reported the same quantized 6/12/24/48 ms
...
Our binary is identified by source base c895265 and md5
`3ab419d2389b00289a9673ede3beace8`; egglog is revision 7b1adf2, md5
`9213d10fe3777cce331a6eba317e3945`. Every number above comes from one fixed copy
of each binary. Node counts are deterministic across runs of those binaries,
verified at n = 20 over five runs.
