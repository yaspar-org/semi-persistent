...
One non-Criterion diagnostic pair measured 11 597 ms against 11 642 ms for the rules
encoding and 1 097 ms against 1 136 ms for the native one; those raw observations do not
support a performance-equivalence claim. Match steps differed by 0.0003%, because the
literal atom costs a join and a payload check where the global costs an equality check.
The identical totals under identical seeding are the semantic statement:
...
forms reapply through the residual nodes they generate rather than constructing a
closed-form comprehension over the whole rest. They therefore materialize intermediate
sub-products and sub-sums explicitly; that cost shows up in the node counts below.
...
`python3 scripts/egglog-compare/compare.py --benchmark math-microbenchmark`
to refresh the medians.
