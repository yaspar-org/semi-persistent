# Historical Comparison Methodology and Divergence Registry

This is the retained account of every decision and divergence in the
egglog-vs-semi-persistent study. Its process-median campaigns are source-pinned
historical evidence, not measurements of the current implementation. A future
comparison should continue to record every semantic difference here, but a
machine-sensitive release claim additionally requires same-revision Criterion
measurements and bootstrap confidence intervals.

Two status words, used with these meanings everywhere in doc/benchmarks/:
...
- egglog: commit 7b1adf2 (egraphs-good/egglog), built `--release` at its
  ...
  doc/benchmarks/README.md. The pinned-toolchain binary is 8.82 MB and
...
- semi-persistent: branch egraph-wf at the commit each campaign's pinning
  ...
  every table states its binary's commit. The semi-persistence tables use
  pinned binary copies with recorded md5s because concurrent rebuilds were
  a risk during that experiment; their record states the digests.
...
The archived campaigns measured process-level wall clock over the release CLIs,
N runs after warmups
...
This protocol retained enough information to describe those executions, but it
did not compute bootstrap confidence intervals. It must not be reused as the
performance gate. New performance comparisons use Criterion around the
same-revision workloads and report its estimates and confidence intervals;
`compare.py` remains useful for provenance and non-timing counters.
...
- **Ruleset default reading.** Our `(run N)` runs the default ruleset
  ...
  experiment.
- **calc.egg is the semi-persistence macro exhibit.** herbie's blocks all run
  under 1 ms on both engines, so the study exhibits multi-block push/pop on `calc.egg`,
  ...
  (doc/benchmarks/records/semi-persistence/semi-persistence.md section 7).
- **The semi-persistence study reports the minimum of the timed runs, not the median.** Its
  ...
  so a median is recomputable. Applies only to that study.
- **The semi-persistence binaries are pinned by digest.** The tables were run
  against fixed binary copies whose hashes are recorded with the results.
...
carriers (array, subsume) are in `doc/benchmarks/README.md`; their
...
- 2026-08-16, restore stopped rebuilding the hashcons index: the ten node
  ...
  kept as a fallback above a quarter of the arena. Bare push/pop at
...
- 2026-08-16, the index build stopped writing its whole key space: the
  ...
  S = 1e6 the semi-persistence round's index build goes 57.61 ms -> 32.64 ms and the
  ...
  and supersedes its "ours, naive" column; the addendum is section 12.
  Corpus byte-identical on 26
...
retained final campaign did use one pinned source snapshot, but it still
predates the current implementation.
...
- Process-startup floors dominate small instances and vary with host load.
  Historical quiet campaigns measured roughly 2-4 ms; loaded-host `final-r6`
  measured roughly 10 ms on our smallest configurations. The sweep reports
  node counts alongside for the asymptotic story, and the growth-shape
  qualification (staircase in iteration count within n=7..20, not a smooth
  curve) is stated where it applies.
...
Every published comparison records new divergences with their justification and
measured consequence. Per-benchmark details stay in the deviation ledgers; this
registry is the cross-benchmark index.

## 9. Campaign records

A campaign is one timed pass over the whole set from one identified binary of
each engine, on one machine with hardware and load recorded. A quiet host is
preferred; a loaded campaign must say so and cannot support isolated-host
absolute timing claims. Each campaign file in `records/campaigns/` carries its
own source and environment block. The newest retained campaign is
`records/campaigns/final-r6-tables.md`; it and r1 through r5 are historical
records whose CSVs stay cited by ledgers and experiment write-ups.

`final-r6` is the first campaign under the raw-data retention protocol: 750 raw
timed invocations, 75 aggregates with dispersion and stable structural
metrics, exact binary SHA-256 hashes, source-tree state, every command,
timestamps, machine/tool metadata, and the full protocol. Egglog's required
statistics are nodes and iterations because revision 7b1adf2 does not expose
e-class counts; ours requires nodes, classes, and iterations. One untimed
normal-mode egglog probe obtains table cardinalities that its timed
`no-messages` JSON omits. The report records a starting load near 6.8 on a
14-core host. It is evidence only for the named historical binaries, without
bootstrap confidence intervals, and is not a quiet-host replication or a
release-performance result.

The r1 campaign's pinning block follows because r2 and r3 refer to it for the
machine.
