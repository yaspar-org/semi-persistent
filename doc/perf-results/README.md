...
| | |
|---|---|
| toolchain | pinned in `rust-toolchain.toml` (currently 1.97.1); each record states the toolchain it ran under |
| cores | 14 (Apple M4 Pro; the machine of `comparison/methodology.md`'s records) |
...
| [E15](E15-rest-binding-write-through.md) | write rest bindings through to the match pools instead of collecting a temporary per binding | **accepted**: 21.5%/17.5% on ac10, 12.5%/9.6% on ac6, 42% fewer allocations |
| [E16](E16-matchset-soa-probe.md) | flat SoA MatchSet probed from the consumer's side | **postponed**: reopened for the results role and landed as E17 |
| [E17](E17-flat-match-store.md) | MatchPool stores its matches in the flat MatchSet | **accepted**: double digits on every match-emitting workload; ac10 allocations and peak memory 64% down |
