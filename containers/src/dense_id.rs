    ...
    Clone + Copy + Default + PartialEq + Eq + Ord + Hash + Tagged + IndexLike + Into<Self::Index>
{
    ...
    /// Convert to the natural storage word.
    #[inline(always)]
    ...
    /// Construct from a representable dense index.
    ///
    /// Generated bounded IDs panic on an out-of-range value. Call
    /// [`DenseId::try_new`] at an untrusted capacity boundary.
    fn from_usize(n: usize) -> Self;

    /// Range-checked construction over the ID payload range, not the wider
    /// backing-word range.
    #[inline(always)]
    fn try_new(n: usize) -> Option<Self> {
        <Self as IndexLike>::try_from_usize(n)
    }

    /// Whether `Tagged` steals a bit from `Self::Index`.
    ///
    /// Every ID generated by this crate reserves its backing word's MSB.
    #[inline(always)]
    fn bit_stealing() -> bool {
        true
    }
}
