...
**The delta index build costs 1.38 ms per round** (S = 1e6,
`comparison/span-table-sparsity.md` section 11.3, which also records the
12.75 ms cost of the rejected dense form). A span table dense over the full
index's key space pays for the key space and not for the delta: a round that
files 9 100 values into 1.02 M keys spends 11.0 ms on the span table against
0.168 ms walking the stream. Building through the arena of
...
completeness premise is: every way a match can become available shows
...
amortized by absorbing the smaller side. Constraint atoms are applied
as filters uniformly across all variants: excluding them from the
decomposition changes nothing about which matches a variant finds,
only how many redundant variants would otherwise run.
...
- A constraint between two atoms' *node* variables, which the
  ...
  tuple it scans for. Measured on the `matrix` translation, whose
...
them. Within a chosen order, leapfrog already adapts to the actual
...
6. **`ByContains` driver-narrowing for variadic atoms (IMPLEMENTED).**
   ...
   by one halving per bound element, the same heuristic
