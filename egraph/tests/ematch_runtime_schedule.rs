...
//! The workload that separates plan-time atom ordering from per-binding atom
//! ordering (design chapter 20, S4).
//!
...
//!
//! The two measurements are match steps (load-independent, every build profile)
//! and wall clock (release only, following `ematch_op_filter`).
...
fn s4_shape(pairs: usize, fan: usize, matched: usize) -> (EG, Query, usize) {
...
/// instead of silently measuring something else. (The sweep does not assert it:
/// at a fan-out of one the probe relations are smaller than `f` and the planner
/// rightly drives from one of them.)
...
fn time(runtime: bool, eg: &EG, query: &Query, reps: usize) -> (f64, usize) {
    let full = IndexStore::build(eg);
    let vindex = VariantIndex::naive(&full);
    let globals = GlobalCtx::<SortId, ENodeId>::new();
    set_runtime_scheduling(runtime);
    ...
    for _ in 0..reps {
        let t = Instant::now();
        matches = run_query_scheduled(&query.rq, &query.plan, eg, &vindex, &globals).len();
...
// ---------------------------------------------------------------------------
// Regression gate: match steps, every profile
// ---------------------------------------------------------------------------
...
/// steps on the heterogeneous shape.
///
/// The factor is what the shape predicts and not a tuning knob: a plan-time
...
/// first and stops. Stated at 10x against a measured 39x so that it fails on a
/// regression rather than on noise; timing-free, so it holds in every build
/// profile.
#[test]
fn runtime_scheduling_cuts_steps_on_the_heterogeneous_shape() {
    let (eg, query, expect) = s4_shape(1024, FAN, 8);
...
fn both_orders_agree_on_the_match_set() {
    for (pairs, fan, matched) in [(64usize, 8usize, 64usize), (256, 32, 16), (16, 128, 0)] {
        let (eg, query, expect) = s4_shape(pairs, fan, matched);
...
// ---------------------------------------------------------------------------
// Regression gate: timing, release codegen only
// ---------------------------------------------------------------------------

/// The step reduction is a wall-clock reduction and not a bookkeeping one.
///
/// Release-only, following `ematch_op_filter`'s canary and the binary-search
/// canary in `containers-conformance`: the per-binding choice costs a handful of
/// bucket-length reads per partial match, and an unoptimized build inflates
/// those out of proportion to the leapfrog iterations they save. The decision
/// half of the gate (the step count above) runs in every profile.
#[test]
fn runtime_scheduling_is_faster_on_the_heterogeneous_shape() {
    ...
    let (eg, query, expect) = s4_shape(1024, FAN, 8);
    assert_static_order(&query.plan);
    let (t_off, m_off) = time(false, &eg, &query, 15);
    let (t_on, m_on) = time(true, &eg, &query, 15);
    assert_eq!((m_off, m_on), (expect, expect), "match sets must agree");
    println!(
        "wall: plan-time {t_off:.3} ms, per-binding {t_on:.3} ms, ratio {:.3}",
        t_on / t_off
    );
    assert!(
        t_on * 2.0 < t_off,
        "expected per-binding scheduling to be more than 2x faster, \
         measured {t_on:.3} ms against {t_off:.3} ms"
    );
}

// ---------------------------------------------------------------------------
// Benchmark: the table recorded in design chapter 20, S4
// ---------------------------------------------------------------------------

/// Print steps and wall for both orders as the populated side's fan-out grows.
/// Ignored by default.
///
/// `cargo test --release -p semi-persistent-egraph --test ematch_runtime_schedule -- \
///  --ignored --nocapture sweep`
#[test]
#[ignore = "sweep: regenerates the chapter 20 S4 table"]
fn sweep() {
    println!(
        "{:>7} {:>6} {:>9} {:>12} {:>12} {:>8} {:>11} {:>11} {:>8}",
        "pairs",
        "fan",
        "order",
        "steps off",
    ...
    for (pairs, fan) in [
        ...
        let (eg, query, expect) = s4_shape(pairs, fan, 8);
        let order = static_order(&query.plan);
        let (m_off, steps_off) = run(false, &eg, &query);
        let (m_on, steps_on) = run(true, &eg, &query);
        assert_eq!((m_off, m_on), (expect, expect), "match sets must agree");
        let (t_off, _) = time(false, &eg, &query, 9);
        let (t_on, _) = time(true, &eg, &query, 9);
        let order = format!("{}-{}-{}", order[0], order[1], order[2]);
        println!(
            "{pairs:>7} {fan:>6} {order:>9} {steps_off:>12} {steps_on:>12} \
             {:>8.2} {t_off:>11.3} {t_on:>11.3} {:>8.2}",
            steps_off as f64 / steps_on as f64,
            t_off / t_on
        );
    }
}
