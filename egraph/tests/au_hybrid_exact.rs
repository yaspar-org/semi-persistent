...
//! Hybrid exact subproblems (plan item A7, doc/au-solver-plan.md).
//!
//! A8 made the certification budget track `sum A(v)`: with the closed bit,
//! MCGS still has to realize every action of every reachable OR node, it just
//! stops wasting playouts on the ones it already realized. A7 removes actions
...
//! in comparison/au/anytime-corpus.md section (g).
//!
...
/// subproblem. It isolates what A7 claims (subproblems are proved instead of
