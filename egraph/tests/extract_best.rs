...
//! real cost.
//!
//! There is no test for `ExtractError::NoGroundTerm`. That needs a class whose
//! every member depends on an unreachable class, and `add` cannot build one: a
//! node's children are ids that already exist, so every class is grounded through
//! the leaves it was built from. That arm is defensive. Its sibling
//! `AllUnextractable` *is* reachable — a `:unextractable` op removes nodes from
//! the candidate set after they were built — and is tested below.
