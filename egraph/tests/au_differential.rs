...
//! Differential gate for AU solver implementations.
//!
//! Solver changes are compared against the exhaustive uninformed exact search
//! (no pruning, no ordering) and the current MCGS. This harness pins their
//! outputs on a deterministic
...
//!
//! The fixture enforces these solver invariants:
//!
//! * Pure data-structure or representation changes keep the fixture
//!   bit-identical.
//! * Ordering and heuristic changes keep every `exact` line identical
...
/// Projection-pruning flag-on check: the
...
/// must come with the exact quality tuple. Dominance pruning drops actions
/// whose lower bound strictly exceeds the node's generalize value (an exact,
/// always-available alternative), so soundness must hold line for line; the
/// MCGS lines themselves MAY move against the flag-off fixture (freed budget
/// explores differently), and this test prints how many moved and in which
/// direction (better / worse / certified earlier / certified lost).
#[test]
fn dominant_pruned_mcgs_is_sound() {
    ...
    for spec in full_specs() {
        ...
        for playouts in PLAYOUT_BUDGETS {
            ...
            assert!(
                ...
                "{id}: dominance-pruned MCGS at {playouts} playouts reports size {}, beating \
                 the exact optimum {exact_size}; the pruning rule is unsound",
                quality.0
            );
            if certified {
                assert_eq!(
                    ...
                    "{id}: dominance-pruned MCGS at {playouts} playouts reports \
            ...
            if (quality, certified) != (golden_quality, golden_certified) {
                moved += 1;
                println!(
                    "{id} mcgs p{playouts}: size={} vmass={} certified={} (fixture: size={} \
                     vmass={} certified={})",
                    quality.0,
                    quality.1,
                    if certified { "yes" } else { "no" },
                    golden_quality.0,
                    golden_quality.1,
                    if golden_certified { "yes" } else { "no" }
                );
...
/// Closed-bit flag-on check: MCGS with
...
/// Hybrid-exact flag-on check, both alone and combined with `closed_bit`. The
...
/// With `live_incumbent_pruning` on
/// (over `closed_bit`, which it requires), the search must stay sound (never
/// beat the exact optimum; `Completion::Exact` only at the exact optimum) and
/// must not lose quality against the flag-off fixture at any budget: an
/// excluded arm is proven non-optimal by its admissible bound against the
/// live incumbent (strict, size-only), so exclusions may only speed the
/// search up, never change what it can find.
#[test]
fn live_incumbent_pruning_is_sound() {
    fn field(value: &str, key: &str) -> String {
        value
            .split_whitespace()
            .find_map(|part| part.strip_prefix(&format!("{key}=")))
    ...
    for spec in full_specs() {
        ...
        for playouts in PLAYOUT_BUDGETS {
            ...
            assert!(
                quality.0 >= exact_size,
                "{id}: live-incumbent pruning at {playouts} playouts reports size {}, beating \
                 the exact optimum {exact_size}; the prune is unsound",
                quality.0
            );
            if certified {
                assert_eq!(
                    quality,
                    (exact_size, exact_vmass),
                    "{id}: live-incumbent pruning at {playouts} playouts reports \
                     Completion::Exact off the exact optimum; an exclusion pruned the true \
                     optimum (the strict size-only rule was violated)"
                );
            }
            ...
            assert!(
                quality <= golden_quality,
                "{id} mcgs p{playouts}: live-incumbent pruning returns {quality:?} against the \
                 flag-off fixture's {golden_quality:?}; excluding proven-non-optimal arms must \
                 not cost quality"
            );
            assert!(
                !golden_certified || certified,
                "{id} mcgs p{playouts}: the flag-off fixture certified and live-incumbent \
                 pruning did not"
            );
            if quality < golden_quality {
                better += 1;
...
/// Rollout-hybrid flag-on check, both alone and combined with `closed_bit`. The
/// trigger replaces the enumeration of a small subproblem with one exact
/// solve of the same state, so the mcgs lines may move but only toward the
/// optimum. The threshold is the instance's own root estimate minus one:
/// `reachable_pairs` is monotone non-increasing down the search graph, so that
...
/// instance absorbed by a single call.
#[test]
fn rollout_hybrid_mcgs_is_sound() {
    fn field(value: &str, key: &str) -> String {
        value
            .split_whitespace()
            .find_map(|part| part.strip_prefix(&format!("{key}=")))
...
/// Session-memo flag-on check, both alone and combined with `closed_bit`. The
/// trigger replaces the enumeration of a small subproblem with one exact
/// solve of the same state, so the mcgs lines may move but only toward the
/// optimum. The threshold is the instance's own root estimate minus one:
/// `reachable_pairs` is monotone non-increasing down the search graph, so that
...
/// instance absorbed by a single call.
#[test]
fn persistent_memo_exact_is_sound() {
    fn field(value: &str, key: &str) -> String {
        value
            .split_whitespace()
            .find_map(|part| part.strip_prefix(&format!("{key}=")))
...
/// Static child seeding must stay
/// sound: never beat the exact optimum, and `Completion::Exact` only at the
/// exact optimum. Matched-playout quality is NOT asserted against the
/// flag-off fixture: the flag deliberately trades estimate quality per
/// playout for cost per playout, and its acceptance is measured at matched
/// wall clock in the corpus, not here. Determinism is asserted: two runs of
/// the same configuration return identical quality and completion.
#[test]
fn static_child_seed_mcgs_is_sound() {
    fn field(value: &str, key: &str) -> String {
        value
            .split_whitespace()
            .find_map(|part| part.strip_prefix(&format!("{key}=")))
    ...
    for spec in full_specs() {
        ...
        for playouts in PLAYOUT_BUDGETS {
            let run = |snap: &AuSnapshot<_, _, false, false>| {
                let r = anti_unify(
                    snap,
                    inst.left,
                    inst.right,
                ...
                .unwrap();
                let vmass = r.pool.variant_mass(r.term_id);
                (r.size, vmass, r.completion)
            };
            let (size, vmass, completion) = run(&snap);
            let again = run(&snap);
            assert_eq!(
                (size, vmass, completion),
                again,
                "{id} p{playouts}: static child seeding is nondeterministic"
            );

            assert!(
                size >= exact_size,
                "{id}: static child seeding at {playouts} playouts reports size {size}, \
                 beating the exact optimum {exact_size}; the seeding is unsound"
            );
            if completion == Completion::Exact {
                assert_eq!(
                    (size, vmass),
                    (exact_size, exact_vmass),
                    "{id}: static child seeding at {playouts} playouts reports \
                     Completion::Exact off the exact optimum"
                );
            }
            checked += 1;
        }
    }
    println!("static_child_seed_mcgs_is_sound: {checked} runs checked");
}
...
/// search up, never change what it can find.
#[test]
fn interval_bounds_mcgs_is_sound() {
    fn field(value: &str, key: &str) -> String {
        value
            .split_whitespace()
            .find_map(|part| part.strip_prefix(&format!("{key}=")))
    ...
    for spec in full_specs() {
        ...
        for playouts in PLAYOUT_BUDGETS {
            ...
            assert!(
                quality.0 >= exact_size,
                "{id}: interval bounds at {playouts} playouts reports size {}, beating \
                 the exact optimum {exact_size}; the prune is unsound",
                quality.0
            );
            if certified {
                assert_eq!(
                    quality,
                    (exact_size, exact_vmass),
                    "{id}: interval bounds at {playouts} playouts reports \
                     Completion::Exact off the exact optimum; an exclusion pruned the true \
                     optimum (the strict size-only rule was violated)"
                );
            }
            ...
            assert!(
                quality <= golden_quality,
                "{id} mcgs p{playouts}: interval bounds returns {quality:?} against the \
                 flag-off fixture's {golden_quality:?}; excluding proven-non-optimal arms must \
                 not cost quality"
            );
            assert!(
