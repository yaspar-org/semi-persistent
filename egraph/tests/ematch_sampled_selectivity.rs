...
/// regression while remaining independent of host timing.
#[test]
fn sampled_selectivity_cuts_steps_on_the_anticorrelated_shape() {
    let (eg, rq, expect) = anticorrelated_shape(LEAVES, HITS, FAN, DISTRACTORS);
...
fn the_bootstrap_guard_rejects_a_draw_one_sample_decides() {
    let (eg, rq, _) = anticorrelated_shape(LEAVES, 1, FAN, DISTRACTORS);
