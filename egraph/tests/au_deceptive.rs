...
//! The deceptive AU instance family and
...
/// MCGS at one playout budget. `dominance` turns on dominance pruning.
pub fn mcgs_quality(inst: &Instance, playouts: u64, dominance: bool) -> Quality {
    mcgs_quality_cfg(inst, playouts, dominance, false)
}

/// MCGS with the full flag set: `live_prune` adds `closed_bit` (which it
/// requires) plus live-incumbent arm pruning.
pub fn mcgs_quality_cfg(
    inst: &Instance,
    playouts: u64,
    dominance: bool,
    live_prune: bool,
...
pub fn playouts_to_optimum(inst: &Instance, target: u32, max_playouts: u64) -> Option<u64> {
    playouts_to_optimum_cfg(inst, target, max_playouts, false)
}

/// `playouts_to_optimum` with live-incumbent pruning when `live_prune` is set.
pub fn playouts_to_optimum_cfg(
    inst: &Instance,
    target: u32,
    max_playouts: u64,
    live_prune: bool,
    ...
    while p <= max_playouts {
        if mcgs_quality_cfg(inst, p, false, live_prune).size <= target {
            return Some(p);
        }
        p *= 2;
    }
...
#[test]
#[ignore = "full deceptive grid: 1.1 s release, prints per-instance misranking arithmetic"]
...
/// With live-incumbent pruning on, the deceptive family's find-side budget
/// collapses.
/// Flag-off, playouts-to-optimum is geometric in burial depth (2 at depth 3,
/// 4096 at depth 20 in the corpus record); flag-on, one composition through
/// the winner excludes every decoy on the path, so a SMALL constant budget
/// must reach the optimum at every depth the smoke grid covers, and the run
/// must certify there (the excluded arms count as resolved).
#[test]
fn live_incumbent_pruning_collapses_deceptive_budget() {
    for &burial_depth in &[3usize, 6, 9, 12] {
        let params = DeceptiveParams {
            burial_depth,
            margin: 4,
            gap: 2,
        ...
        let flag_on = playouts_to_optimum_cfg(&inst, plan.optimum, 64, true);
        assert!(
            flag_on.is_some_and(|p| p <= 16),
            "{params:?}: live-incumbent pruning needed {flag_on:?} playouts to find the \
             optimum; the exclusion is not collapsing the deceptive budget"
        );

        // The prove side: at a modest ladder budget the run certifies, and at
        // the exact optimum (the strict rule kept the true optimum alive).
        let q = mcgs_quality_cfg(&inst, 256, false, true);
        assert!(
            q.certified,
            "{params:?}: 256 playouts with live-incumbent pruning did not certify"
        );
        assert_eq!(
            q.size, plan.optimum,
            "{params:?}: certified at size {} instead of the planted optimum {}",
            q.size, plan.optimum
        );
    }
}
