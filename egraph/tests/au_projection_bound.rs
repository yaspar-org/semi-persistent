...
//! Randomized test of the projection identity and the projection lower bound
//! (plan item A1, doc/au-solver-plan.md), against the exact solver.
//!
...
//!   `TermPool::project`;
//! * the bound `size(t) >= lb_pair(l_root, r_root).0`, the primitive A2's
//!   branch-and-bound prunes with (`au::estimates::lb_pair`).
//!
//! The identity is what makes `lb_pair` admissible (soundness argument A1 in
//! the plan), so pinning both against the real solver checks the theorem and
