...
//!
//! Unit (identity) declarations are excluded from the default corpus: the
//! exact solver panics on identity-degenerate graphs until defect D2 is fixed
//! (au-defects.md on branch egraph-wf, "AC identity class containing an AC
//! member"). The D2 contract case runs in the default suite now that the
//! fix landed; unit declarations stay out of the RANDOM corpus pending a
//! units-aware expected-lgg model.
//!
//! Per the review of 2026-08-14 (au-review.md, D1), MCGS can hang in the f64
//! transport solve. Every generated case therefore runs on a worker thread
...
/// hang in the f64 transport solve (defect D1, au-review.md 2026-08-14). On
...
fn run_case_guarded(seed: u64, playouts: u64, timeout: Duration) -> PairOutcome {
    ...
    match rx.recv_timeout(timeout) {
        ...
        Err(mpsc::RecvTimeoutError::Timeout) => panic!(
            "seed {seed:#018x}: case exceeded {timeout:?}; matches the D1 MCGS transport \
             hang (au-review.md); the worker thread is leaked"
        ),
...
/// Contract test for defect D2 (au-defects.md): an AC identity class that
...
/// for the completion-check performance work (au-review.md, performance
/// item 1): a geometric-schedule completion check must not regress these
