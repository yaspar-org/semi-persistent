...
`subsume.deviations.md`). They never enter a campaign: neither is in
`run-full.py`'s benchmark table, and `run-full.py` additionally refuses
any listed program whose first three lines contain `BLOCKED:`, so
promoting a carrier requires both removing the header and adding the
table row. Each ledger states the condition that unblocks it. `resolution`'s native column is the first ACI
...
`gen-herbie.py` regenerates herbie's egglog and rules programs and
...
`semi-persistence/semi-persistence.md`; its sections 11 and 12.3 state what
...
`hotpath-audit.md` is the dated instruction-level audit that located the
matching hot-path defects (fixed; the fixes and measurements are in
`methodology.md` §6 and design chapter 20). The three `throughput-gap-*.md`
files are the paired profiles behind the matching-throughput comparison:
`throughput-gap-ours.md` and `throughput-gap-egglog.md` profile each
engine, `throughput-gap-synthesis.md` is the side-by-side verdict on the
(closed) 22.7x rules-encoding gap.
...
python3 run-full.py --benchmark <name> ... # any subset, same protocol
```
