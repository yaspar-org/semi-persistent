...
BENCHMARKS = {
    ...
    # Second survey pass. resolution's native column is ACI (:assoc-comm-idem
    # on myor); acgen is generated by egglog-bridge/examples/make-ac-bench.py
    # at N=11 (see acgen.deviations.md for the invocation and N=13 numbers).
    "combinators":         {"types": "machine",        "encodings": ("rules",)},
    "resolution":          {"types": "machine",        "encodings": ("rules", "native")},
    "typecheck":           {"types": "machine",        "encodings": ("rules",)},
    "knapsack":            {"types": "machine",        "encodings": ("rules",)},
    "levenshtein-distance": {"types": "machine",       "encodings": ("rules",)},
    "intersection":        {"types": "machine",        "encodings": ("rules",)},
    "acgen":               {"types": "machine",        "encodings": ("rules", "native")},
}


def blocked(path):
    """True when the program's first three lines carry a BLOCKED: header.

    BLOCKED carriers (array, subsume) are translations that cannot run or
    cannot be compared on this engine yet; their ledgers name the missing
    feature. They live in this directory but never enter a sweep or campaign.
    """
    with open(os.path.join(HERE, path)) as fh:
        return any("BLOCKED:" in (fh.readline() or "") for _ in range(3))
...
def main():
    ...
    for b in (args.benchmark or list(BENCHMARKS)):
        for name, argv, stats_path, flavour in configs(b, ours, args.egglog):
            program = argv[-1] if flavour == "egglog" else argv[1]
            if blocked(program):
                print(f"skip {program}: BLOCKED header", file=sys.stderr)
                continue
