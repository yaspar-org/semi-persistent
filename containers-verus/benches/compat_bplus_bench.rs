...
fn label<L: NodeLayout, S: SearchKind>(key_width: &str) -> String {
    ...
    // The verus layouts have no NODE_SIZE const (production's byte size);
    // label by leaf capacity, which identifies the layout uniquely per width.
    format!("bplus_cap{}_{}_{}", L::leaf_cap(), key_width, search)
}
            ...
            b.iter(|| {
                ...
                for &target in seeks {
                    ...
                    if let Some(k) = cur.key() {
                        sum = sum.wrapping_add(k.as_usize());
                    }
                }
