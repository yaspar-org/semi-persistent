    ...
    I: IndexLike,
{
    ...
    fn push(&mut self, value: T) {
        ...
        // Reading the live `Vec` length surfaces vstd's axiom that a real
        // `Vec`'s `@.len()` fits in `usize`. With `captured@.len() == old
        // data@.len() == data@.len() - 1`, that bounds the bit-vector's length
        // `< usize::MAX`, so its push can't overflow the length counter.
        let _len_fits_usize = self.data.len();
        ...
        where T: core::default::Default
    {
            ...
            decreases target - self.data.len(),
        {
            ...
            // Vec length axiom: the live `data` length fits `usize`, so
            // `captured`'s (equal) length is `< usize::MAX` and its push is safe.
            let _len_fits_usize = self.data.len();
            self.captured.push(false);
        }
    }
