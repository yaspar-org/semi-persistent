# Implementation Plan — Pop into a marked region, with `T: Copy + Default`

Status: **COMPLETE** (verifies clean, no admits). All
five steps landed in order, each leaving the tree green — see
`08-proof-attempts-log.md` "Pop into the marked region — how it landed" for the commit-by-commit
...
References: `01-verification-design.md` §8 (pop into the marked region), §6 (invariant),
...
**Recommendation:** do **`Copy + Default`** for the marked-region-pop proof (keeps
...
Per §8 of the design doc, pop into the marked region requires relaxing **two** wf clauses,
...
- Restate the central lemma to prove
  ...
  handling cells `j ∈ [saved_{k+1}, saved_k)` via the **captured** case (frame
...
prove wf_for_snap holds on the resized state   // resize preserves it (captured
                                          //   case layer-independent; coverage
...
fillers in the gap; coverage says those cells are captured so the captured case
holds and the uncaptured case never reads a filler). This was prototyped before;
...
5. **`pop` into the marked region** — conditional capture; drop the
   precondition.
