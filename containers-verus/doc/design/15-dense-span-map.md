...
`DenseSpanMap<V>` stores, the three properties its proofs establish, and the
rationale for its proof structure. `cargo verus verify` reports the current
per-module tally, and CI asserts the trust-surface counts.
...
filtered sequences. An over-broad `requires` would drag unused quantifiers into
every proof that mentions it.
...
(`forall|i, j| ... i != j ==> ranges disjoint`) creates quadratic
instantiations because a trigger set with two
disjoint bound-variable groups instantiates over every pair of matching terms.
The pairwise fact is therefore derived on demand.
...
placement step), `lemma_spans_disjoint`, `lemma_spans_monotone`,
...
The dense build verifies with the default solver budget and no custom
`#[verifier::rlimit]`.

### Relevant vstd API details
...
**The filter-subset lemma is provided by vstd.** The push case of
`lemma_filter_sorted` needs "every element of a filtered sequence came from the
sequence". vstd supplies this as `Seq::lemma_filter_contains_rev`.
...
`call_ensures`-shaped obligation (`std_specs/slice.rs`). `split_at` is used
...
unchanged at 27, and the `literal-types` build remains at 32 total (five
additional opaque type registrations). Nothing in it is added to the crate's
explicit trust ledger. Like every Verus proof, it still relies on Verus/vstd,
including the `Vec` and slice specifications, and on ghost erasure; those are
the ambient verification substrate, not ledger Group A (which is the
crate-local `ContainerId` boundary). There are no `admit`s or `assume`s.
...
The differential tests cover randomized and key-skewed streams, keys with no entries,
...
A fresh dense build grows the span table to `num_keys`, however few values its
stream carries. The counting, extent, and placement passes themselves touch
the stream and occupied keys; the fresh table growth contributes O(num_keys).
Thus `build` is O(stream + occupied + num_keys). Section 4 of
`16-layered-span-map.md` states the same fresh-table term for layered delta
installation.
...
and reads as empty. `recycle` hands the arena back. If the retained table is
already at least `num_keys` long, an ordinary build is O(stream + occupied).
Growing from `old_table_len` adds
O(max(0, num_keys - old_table_len)). After `u64::MAX` positive generations,
the following build performs an exceptional O(table length) clear before
restarting at generation 1.

The e-graph's four index families build through `build_in`
(`egraph/src/index.rs`). Performance evidence for this path belongs in the
Criterion index and saturation benchmarks; this chapter makes no fixed timing
claim.

`LayeredSpanMap::replace_delta` is a separate case and still builds each delta
with a fresh arena, so its complexity retains the O(num_keys) table-growth term
documented in chapter 16.

**There is one implementation path, not two.** `build` delegates to `build_in` with a fresh
...
generation is always positive. After the `u64::MAX` available positive
generations have been used, the next build re-stamps the whole table to 0 and
resets the generation to 1, O(num_keys). The branch is written and proved rather
than argued away, because "unreachable in practice" is not a postcondition. It
is also not runtime-tested: reaching it requires `u64::MAX + 1` builds from a
new arena, so the conformance suite cannot drive it, and that is a consequence
of choosing `u64` over the prototype's `u32`.

A `{off, len, stamp}` triple has a larger resident span-table entry than an
unstamped span. Recycling avoids clearing and reallocating the complete table
on ordinary builds, but resident span-table memory remains proportional to the
largest key space for which that arena has been grown. The net memory and
throughput tradeoff is workload and allocator dependent and must be evaluated
with the Criterion index/saturation benches.

### Proof decomposition

The three passes are contracted functions verified in isolation: `count_pass`,
`extent_pass`, and `place_pass`, composed by `build_in`. `permute` is an opaque,
named spec function with `lemma_permute_index` and `lemma_permute_len` as its
interface; an inline `map_values` closure would be re-elaborated at every loop
invariant occurrence. The tiling conclusion lives in `place_pass`, next to the
loop invariant that carries its premises. `build_in` uses `spinoff_prover` for
the final composition and no custom `rlimit`.

Verus loops see only their invariant, not the enclosing function's
preconditions, so required facts are restated in the loop invariant.
