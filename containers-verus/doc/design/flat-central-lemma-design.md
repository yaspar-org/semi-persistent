...
`snapshots[target]` cell-by-cell via the two arms.
...
- If the target's stratum does NOT hit `j`: target's frame_inv *uncaptured*
  arm gives `j < layer_above(target).len()` and `layer_above(target)[j] ==
...
saved_{target+1}`), so it falls in the captured arm and the recursion stops at
...
`frame_inv_range` (captured arm gives the entry's value; uncaptured arm steps
...
- `target_cell` downward induction: medium risk — it's a reformulation, but the
  ...
  and every step is a direct `frame_inv_range` arm application (we already have
