...
and, for each, why it is trusted rather than proved. As of this writing there
are **6 such items**, in two groups.*
...
boundary. This crate started with ~16 such functions and now has 6; the journey
is recorded in [Ch. 8](08-proof-attempts-log.md). The ones that were
*eliminated* (the `IndexLike`/`DenseId` integer casts) are described in §3; the 6
that *remain* are §1 (Group A) and §2 (Group B).
...
[Ch. 2 §4c](02-fork-history.md)) — but (i) the atomic read itself would remain
...
safety ([Ch. 2](02-fork-history.md)) hold *without* it; the container id only
...
The lesson (recorded in [Ch. 8](08-proof-attempts-log.md)): "wraps a cast" is not
