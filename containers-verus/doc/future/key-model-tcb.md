...
*Status: future work. Owner: whoever migrates `LitValStore`.
Context: trust ledger group D
([02-trust-boundary.md](../design/02-trust-boundary.md) §3.5); the
BigRational/OrderedFloat key-model axioms are withdrawn as false, with the
...
Shipped in the verified crate (recorded here because the remaining plan builds on them):
...
## Float term identity: the fold is parity, not the endgame
...
## Axiom inventory for the consumer conversion: nine key types, not one
