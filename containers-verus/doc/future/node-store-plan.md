...
`EGraph`'s persistent state decomposes as:

| state | storage | logic |
|---|---|---|
| `classes: EClasses` | verified | verified (W1-W7) |
| registries, `LitValStore`, `unit_node`, `inverse_op` | source rows stored in verified `SpMap`/`VecI` containers | container transitions verified; cross-column registry/model agreement remains ordinary Rust logic |
...
The persistent source-of-truth node rows and history columns live in verified
containers. Derived raw hash-cons indexes and the logic layered over all of
these structures are not thereby verified: the remaining obligations are the
routing bijection, hash-cons agreement, and two-phase id mint. Those are the
...
- **N4, hash-cons coverage up to the dirty set.** Every stored row's
  ...
  the `wf_except(dirty)` discipline (egraph-wf D1); rebuild's job is
  restoring `dirty = empty`.
- **N5, history archival.** Under `PROOFS`, recanonicalization history rows
  compose with mark/restore the way every archived container does.

R1 (no two live rows congruent at the fixpoint, `egraph-wf.md`) is the
rebuild theorem, not a step invariant. N3+N4 are its preconditions;
landing them makes R1 STATEABLE, which it currently is not.
...
**S0: monitors (days).** Stage-0 pattern: debug-build asserts after
...
**S1: verified keyed cache (one to two weeks).** A `containers-verus`
...
update as one contracted operation, mark/restore with per-frame archives). Generic over
...
**S2: verified router (days).** `TypedRouting` moves into the kernel:
...
**S3: the NodeStore aggregate (the big one; eclasses-scale).** Compose
...
joint archive keyed on component snapshot stacks, total public
...
**S4: the swap (days, given S3).** `node_store.rs`, `caches.rs`,
`typed_routing.rs` follow the `classes.rs` pattern (aliases of the kernel
container, no adapter layer): production signatures, pinned panic
messages, compile-time layout asserts, zero
...
**S5: `wf_except(dirty)` for the whole EGraph (the precondition for
D1/R1).** With W1-W7 and N1-N3 both machine-checked, state the dirty set
as a spec object: `merge` adds to it (N4's exception set), each rebuild
iteration shrinks it, empty dirty set implies full `wf`. This is the loop
invariant a verified rebuild needs and the point where R1 becomes a
statable theorem. Proving R1 itself remains postponed per egraph-wf.

## Order and what would change it

S0 first, always. S1 before S2 only because the cache is the riskier
...
S3 requires both; S4 requires S3; S5 requires S4 (the class-layer
aggregate W1-W7 it also needs is shipped). If S0's monitors fire on the
existing suites, stop and fix: a violated N2 or N3 means the informal
argument is wrong and S3's proofs would chase a falsehood. If S1's
wrapped-index contract costs measurable throughput on `saturate_bench`
(the intern path is the hot path of `add_*`), the fallback is recorded
now: keep the raw index consumer-side and land S3 with the index clauses
stated over a ghost map the adapter
