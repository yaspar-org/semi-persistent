...
*Bring the byte-accounting family (`tracking_bytes` / `total_bytes` /
`heap_bytes`, currently `#[verifier::external_body]` across the containers
and stores) under a verified spec, so no exec code stays trusted merely
because a spec was never written. Companion to the trust-boundary
...
The same shape recurs in `ListArena` and `CircularList`
(`tracking_bytes`/`total_bytes`) and in the store backends
(`ParallelStore`/`InlineStore`/`CaptureBits` `heap_bytes`); grep for
`external_body` next to `bytes` for the current inventory.
...
4. **Scope.** `heap_bytes` on the inner stores
   ...
   (self-contained), then `ListArena`/`CircularList`, then the store
   backends.
