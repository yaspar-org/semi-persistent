...
Most fold and transform schemes generated by `rec_family!` take one
closure per sort, in declaration order. Multi-phase schemes have
explicit grouping: `prefold` takes all preprocessors before all
algebras; `fold_with_aux` takes all aux algebras before all main
algebras; and `fold_pair` takes the A and B algebras together for each
sort.

Construction schemes differ. `unfold` and `unfold_short` take one
sort-tagged coalgebra for the whole family. `postunfold` takes one
postprocessor per sort, followed by that tagged coalgebra.
